...
def test_get_last_download_with_days_back():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert abs((three_days_ago - expected.replace(hour=0, minute=0, second=0, microsecond=0)).total_seconds()) < 60
        
        # Test with days_back=7
        seven_days_ago = tracker.get_last_download(days_back=7)
        expected = datetime.now() - timedelta(days=7)
        
        assert abs((seven_days_ago - expected.replace(hour=0, minute=0, second=0, microsecond=0)).total_seconds()) < 60
